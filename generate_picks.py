...
vesicle_picks.add_fields(['location/micrograph_uid', 'location/exp_group_id', 'location/micrograph_path', 
                          'location/micrograph_shape', 'location/center_x_frac', 'location/center_y_frac', 'location/micrograph_psize_A'],
...
for micrograph in tqdm(micrographs[0:]):
    ...
    masks_filename = parameters.get('input', 'directory')+str(uid)+"_vesicles_filtered.pkl"

    # If the mask filename isn't in the input directory, then go to the next micrograph
    if not os.path.isfile(parameters.get('input', 'directory')+str(uid)+"_vesicles_filtered.pkl"):
        continue
    ...
    if int(parameters.get('picking', 'dilation_radius')) < 0:
        masks = postprocess.erode_masks(masks, -int(parameters.get('picking', 'dilation_radius')), float(parameters.get('general', 'psize')), int(parameters.get('general', 'downsample')))
    elif int(parameters.get('picking', 'dilation_radius')) > 0:
        masks = postprocess.dilate_masks(masks, int(parameters.get('picking', 'dilation_radius')), float(parameters.get('general', 'psize')), int(parameters.get('general', 'downsample')))
    else:
        # Re-find the edges for picking, since they're lost on saving
        masks = [postprocess.find_contour(mask) for mask in masks]

    pick_indices = postprocess.generate_picks(masks, float(parameters.get('general', 'psize')), int(parameters.get('general', 'downsample')), int(parameters.get('picking', 'box_size')), mode=parameters.get('picking', 'mode'))
...
job = project.create_external_job(parameters.get('csparc_input', 'WID'), title="Vesicle Picks")
