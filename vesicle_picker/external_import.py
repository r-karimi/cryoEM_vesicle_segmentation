from cryosparc.tools import CryoSPARC, Path
...
def load_cryosparc(csparc_login_path):
    ...
    """
    Initialize a cryosparc session with the credentials in a csparc_login.ini file.

    Arguments:
    csparc_login_path (str): Absolute or relative filepath to the csparc_login file containing email, password, etc.

    Outputs:
    cs (cryosparc object): A valid cryosparc session for further use downstream. 
    """
...
def micrographs_from_csparc(cs, project_id, job_id, job_type):
    ...
    job_type: The job from which micrographs are being imported. Can be either a Curate Exposures job ("curate") or a patch CTF job ("patch_ctf").

    Outputs:
    micrographs (cryosparc Micrographs object): A collection of micrographs and associated parameters as a cryosparc object.
    """
...
def import_masks_from_disk(filename):
    ...
    """
    Import a pickle object containing masks compressed by export_masks_to_disk().

    Arguments:
    filename (str): The filename of the compressed masks on disk.

    Outputs:
    masks (list): The uncompressed masks with the 'segmentation' key repopulated and prepared for downstream analysis.
    """
    ...
    for mask in loaded_masks_pickle['masks']:
        mask['segmentation'] = loaded_masks_pickle['composite_mask']%mask['prime_key']==0
