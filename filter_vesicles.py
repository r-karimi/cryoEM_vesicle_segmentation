...
for micrograph in tqdm(micrographs[0:12]):
    ...
    masks_filename = parameters.get('input', 'directory')+str(uid)+"_vesicles.pkl"
    ...
    filtered_masks_filename = parameters.get('output', 'directory')+str(uid)+"_vesicles_filtered.pkl"
    
    # Save the vesicles to the output directory on a micrograph-by-micrograph basis
    external_export.export_masks_to_disk(filtered_masks, filtered_masks_filename, micrograph_uid=uid, compression='uint16')
