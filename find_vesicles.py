from vesicle_picker import preprocess, generate_masks, postprocess, helpers, external_import, external_export
from tqdm import tqdm, trange
import argparse

# Read in the job parameters file
parser = argparse.ArgumentParser(description='Description of your script')
parser.add_argument('parameters', type=str, help='Path to an .ini file containing the job parameters for vesicle picking.')
...
model = generate_masks.initialize_model(model_weights_path = parameters.get('segmentation', 'model_weights_path'),
...
for micrograph in tqdm(micrographs[(5500):]):
    ...
    masks = generate_masks.generate_masks(preprocessed_micrograph, model,
                                          ...
                                          stability_score_thresh=parameters.getfloat('segmentation', 'stability_score_thresh'),
                                          crop_n_layers=parameters.getint('segmentation', 'crop_n_layers'),
                                          crop_n_points_downscale_factor=parameters.getint('segmentation', 'crop_n_points_downscale_factor'),
                                          crop_nms_thresh=parameters.getfloat('segmentation', 'crop_nms_thresh'),
                                          min_mask_region_area=parameters.getint('segmentation', 'min_mask_region_area'))

    if len(masks)==0:
        continue

    # Use the postprocess module to compute statistics on the vesicles for downstream filtering
    ...
    # Directly modify the ellipse fitting key-value pairs to convert to Angstrom
    for mask in postprocessed_masks:
        if 'average_radius' in mask:
            mask['average_radius_A'] = mask['average_radius']*parameters.getfloat('general', 'psize')*parameters.getint('general', 'downsample')
            mask['semi_minor_A'] = mask['semi_minor']*parameters.getfloat('general', 'psize')*parameters.getint('general', 'downsample')
            mask['semi_major_A'] = mask['semi_major']*parameters.getfloat('general', 'psize')*parameters.getint('general', 'downsample')

    # Construct the filepath of the masks to save to disk
    masks_filename = parameters.get('output', 'directory')+str(uid)+"_vesicles.pkl"
    
    # Save the vesicles to the output directory on a micrograph-by-micrograph basis
    external_export.export_masks_to_disk(postprocessed_masks, masks_filename, micrograph_uid=uid, compression='uint64')
